...
export default function Navigation() {
  ...
  // Sperrt das Scrollen, wenn das mobile Menü offen ist
  useEffect(() => {
    if (isMobileMenuOpen) {
  ...
  const menuItems = [
    { id: 'home', label: 'Home' }, // Wird zu "/"
    { id: 'about', label: 'Über Uns' }, // Wird zu "/#about"
        ...
        <div className="flex justify-between items-center h-20 relative z-[110]"> 
          ...
          <div className="hidden lg:flex space-x-1 items-center">
            {menuItems.map((item) => (
              <a
                key={item.id}
                // HIER IST DIE VEREINFACHUNG: Normale Links wie im Footer
          ...
          <div className="flex-1 flex flex-col items-center justify-start px-4 space-y-1 overflow-y-auto">
            {menuItems.map((item) => (
              <a
                key={item.id}
                href={item.id === 'home' ? '/' : `/#${item.id}`}
                // WICHTIG: Menü schließen beim Klicken
                onClick={() => setIsMobileMenuOpen(false)}
